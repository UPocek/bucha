...
export default function Kombuha() {
    ...
    const t = useTranslations('storefront.flavors');
    const productTranslations = useTranslations('products.names');
    const products = ['hibiscus'].map((productId) => ({
        productId,
    ...
    return (
        <>
            <SeoHead title={title} description={description} pathname="/kombuha" structuredData={structuredData} />
            <NavWrapper />
            <main
                role="main"
